...
- `/config set_maintenance_for <minutes>` 実行時刻から指定分だけメンテナンスモード有効化
- `/config add_exclude_user <member>` 集計除外ユーザー追加
- `/config remove_exclude_user <member>` 集計除外ユーザー削除
- `/config clear_exclude_users` 集計除外ユーザー全削除
